...
/* Formatted on 2018-11-22 22:22:48 (QP5 v5.336) */
...
begin
    ...
    loop
        ...
        then
            ...
            exception
            when others then
                raise_application_error(-20021,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
            end;
            ...
            then
                ...
                exception
                when others then
                    raise_application_error(-20022,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                ...
                exception
                when others then
                    raise_application_error(-20023,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_sequence := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',2));
                exception
                when others then
                    raise_application_error(-20024,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_status := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',3));
                exception
                when others then
                    raise_application_error(-20025,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_result := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',4));
                exception
                when others then
                    raise_application_error(-20026,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
    ...
    loop
        ...
        then
            ...
            exception
            when others then
                raise_application_error(-20031,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
            end;
            ...
            then
                ...
                exception
                when others then
                    raise_application_error(-20032,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                ...
                exception
                when others then
                    raise_application_error(-20033,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_sequence := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',2));
                exception
                when others then
                    raise_application_error(-20034,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_status := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',3));
                exception
                when others then
                    raise_application_error(-20035,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                begin
                    v_test_value_result := to_number(regexp_substr(v_result,'^TV_(\d+)_(\d+)_(0|1)_(-{0,1}\d+\.{0,1}\d{0,})$',1,1,'',4));
                exception
                when others then
                    raise_application_error(-20036,'result_'||substr('00'||v_i,-3)||' = '||v_result||' is incorrect. Oracle error:'||SQLCODE||' ('||SQLERRM||')');
                end;
                if (v_test_order is null or v_test_value_status is null or v_test_value_result is null)
                then
                    v_test_order := null;
